...
def main(context):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train2(train_loader, model, ema_model, optimizer, epoch, training_log)
...
def train(train_loader, model, ema_model, optimizer, epoch, log):
    ...
    for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        assert not (torch.isnan(loss) or loss.item() > 1e5), 'Loss explosion: {}'.format(loss.item())
...
def train2(train_loader, model, ema_model, optimizer, epoch, log):
    global global_step
